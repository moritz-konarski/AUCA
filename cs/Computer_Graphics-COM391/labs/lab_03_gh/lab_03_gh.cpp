...
int main(int argc, char **argv)
{
    ...
    checkForErrors(errString, "l. 156");
    ...
    for (;;) {
        ...
        triangle_model_matrix = glm::rotate(triangle_model_matrix, 
                triangle_rotation.y, glm::vec3(0, 1, 0));
        triangle_model_matrix = glm::rotate(triangle_model_matrix, 
                triangle_rotation.x, glm::vec3(1, 0, 0));
        triangle_model_matrix = glm::rotate(triangle_model_matrix, 
                triangle_rotation.z, glm::vec3(0, 0, 1));
        triangle_model_matrix = glm::scale(triangle_model_matrix, 
                triangle_scale);

        camera_model_matrix = glm::translate(glm::mat4(1.0f), 
                camera_position);
        camera_model_matrix = glm::rotate(camera_model_matrix, 
                camera_rotation.y, glm::vec3(0, 1, 0));
        camera_model_matrix = glm::rotate(camera_model_matrix, 
                camera_rotation.x, glm::vec3(1, 0, 0));
        camera_model_matrix = glm::rotate(camera_model_matrix, 
                camera_rotation.z, glm::vec3(0, 0, 1));
        ...
        if (first_time) {
            std::cout << glm::to_string(triangle_model_view_projection_matrix) 
                << std::endl;
            first_time = false;
        }

        checkForErrors(errString, "l. 279");
        ...
        checkForErrors(errString, "l. 284");
